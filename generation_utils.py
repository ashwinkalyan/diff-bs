...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs
        ...
        Parameters:
            ...
            diversity_strength (obj: `float`, `optional`, defaults to 0):
                weightage for the diversity term, implements diverse beam search when > 0
        ...
        do_dbs = diversity_strength > 0 if diversity_strength is not None else self.config.do_sample
        ...
        is_dbs_gen_mode = (num_beams > 1) and do_diverse is True 
    ...
    def diverse_beam_search(
        ...
        **model_kwargs
        ...
        beam_scores_dict = dict()
        input_ids_dict = dict()
        next_tokens, next_indices = dict(), dict()
        for group_idx in range(num_groups):
            input_ids_dict[group_idx] = input_ids.clone()
            beam_scores_dict[group_idx] = torch.zeros((batch_size, num_beams), dtype=torch.float, device=input_ids.device)
            beam_scores_dict[group_idx][:, 1:] = -1e9
            beam_scores_dict[group_idx] = beam_scores_dict[group_idx].view((batch_size * num_beams,))

        while cur_len < max_length:
            for group_idx in range(num_groups):

                model_inputs = self.prepare_inputs_for_generation(input_ids_dict[group_idx], **model_kwargs)
                outputs = self(**model_inputs, return_dict=True)
                next_token_logits = outputs.logits[:, -1, :]
                ...
                next_token_scores = next_token_scores + beam_scores[:, None].expand_as(next_token_scores)
                # add diversity
                next_token_scores = self.add_diversity(next_token_scores, input_ids_dict, group_idx, cur_len, beam_size) # reduce probability of previously occuring tokens
                # reshape for beam search 
                vocab_size = next_token_scores.shape[-1]
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
                ...
                next_indices_dict[group_idx] = next_indices
                next_tokens_dict[group_idx] = next_tokens

                # stateless
                beam_outputs = beam_scorer.process(
                    input_ids_dict[group_idx],
                    next_token_scores,
                    next_tokens_dict[group_idx],
                    next_indices_dict[group_idx],
                    pad_token_id=pad_token_id,
                    eos_token_id=eos_token_id,
                )
                beam_scores[group_idx] = beam_outputs["next_beam_scores"]
                beam_next_tokens = beam_outputs["next_beam_tokens"]
                beam_idx = beam_outputs["next_beam_indices"]

                input_ids_dict[group_idx] = torch.cat([input_ids_dict[group_idx][beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
                cur_len = cur_len + 1
        ...
        decoded_dict = dict()
        for group_idx in range(num_groups):
            decoded[group_idx] = beam_scorer.finalize(
                input_ids_dict[group_idx], beam_scores, next_tokens_dict[group_idx], next_indices_dict[group_idx], pad_token_id=pad_token_id, eos_token_id=eos_token_id
        )

        return [decoded_dict[key] for key in decoded_dict.keys()]

        def add_diversity(joint_log_probs, input_ids_dict, group_idx, cur_len, beam_size):
            """ 
            """
            diversity_strength = self.diversity_strength
            vocab_size = next_token_scores.shape[-1]
            batch_beam_size, _ = input_ids[0].shape
            batch_size = batch_beam_size // beam_size
            div = torch.zeros(beam_batch_size, vocab_size)
            for group in range(group_idx-1):
                idx_tensor = input_ids_dict[group][:, cur_len]
                div[:, idx_tensor] += 1
            joint_log_probs[group_idx] = joint_log_probs[group_idx] - diversity_strength*div
